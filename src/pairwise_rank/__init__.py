"""pairwise_rank: small research tool for repeated balanced pairwise ranking.

Architecture (v0.4, 2026-08-28):
...
Verdict scale:
    ...
    information is essentially unused in practice (STRONG verdicts
    occur in <= 2% of observations; BTD and the ordered logit give
    r_theta > 0.99 and r_P(best) > 0.99 on multiple tournaments).
    Existing 5-level data on disk loads fine and is collapsed on
    use by BTD; no migration is required.
